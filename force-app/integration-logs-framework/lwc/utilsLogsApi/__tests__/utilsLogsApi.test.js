...
describe('logsApi service', () => {
    ...
    describe('fetchPage', () => {
        ...
        it('should not cache result if different params are used', async () => {
            ...
            mockApexFn.mockResolvedValueOnce(mockData1).mockResolvedValueOnce(mockData2);
        ...
        it('should force refresh cache when force: true is passed', async () => {
            ...
            const result2 = await logsApi.fetchPage(mockApexFn, params, { force: true });
        ...
        it('should include correlationId in Apex payload', async () => {
            const mockData = { records: [], hasMore: false };
            mockApexFn.mockResolvedValue(mockData);

            await logsApi.fetchPage(mockApexFn, { pageSize: 20 });

            const call = mockApexFn.mock.calls[0][0];
            expect(call).toHaveProperty('correlationId');
        ...
        it('should respect custom TTL option', async () => {
            ...
            mockApexFn.mockResolvedValueOnce({ records: [{ Id: '2' }], hasMore: false });
            const result = await logsApi.fetchPage(mockApexFn, params);
    ...
    describe('clearCache', () => {
        it('should clear entire cache when no pattern provided', async () => {
            ...
            await logsApi.fetchPage(mockApexFn, { pageSize: 20, statusFilter: 'All' });
            await logsApi.fetchPage(mockApexFn, { pageSize: 50, statusFilter: 'New' });
        ...
        it('should clear cache entries matching a pattern', async () => {
            ...
            await logsApi.fetchPage(mockApexFn, { pageSize: 20, statusFilter: 'All' });
            await logsApi.fetchPage(mockApexFn, { pageSize: 20, statusFilter: 'New' });
